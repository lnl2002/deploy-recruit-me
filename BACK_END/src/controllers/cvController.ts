...
const cvController = {
    ...
    createCV: async (req: Request, res: Response, next: NextFunction): Promise<Response> => {
        try {
            // Validation is handled by the middleware
            const { email, lastName, firstName, gender, phoneNumber, address } = req.body

            const { file } = req
            const key = Buffer.from(process.env.ENCRYPTION_KEY, 'hex')
            const iv = Buffer.from(process.env.INITIALIZATION_KEY, 'hex')

            const encryptedBuffer = await cvService.encryptFileInMemory(file.buffer, key, iv)

            const fileName = `${Date.now()}_encrypted_cv` // Generate a unique file name
